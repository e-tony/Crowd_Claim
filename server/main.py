    ...
    with torch.no_grad():
        if attention_mask:
            x_encoded_layers, pooled_output = model(
                x, x_seg, attention_mask=attention_mask
            )
        else:
...
def get_bert_embedding(text, model, tokenizer, batch_size=-1, device="cuda:0"):
    all_sens = text.split(". ")
...
def run(user_input):
    ...
    embedding_user, lens_user, mask_user, idf_user = get_bert_embedding(
        user_input, model, tokenizer, device=device
    )
    ...
    for i, cat_id in enumerate(CATEGORIES):
        ...
        embedding_c, lens_c, mask_c, idf_c = get_bert_embedding(
            desc, model, tokenizer, device=device
        )
